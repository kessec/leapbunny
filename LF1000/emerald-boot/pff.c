...
)
{
	...
	if (dj->sect) {
		...
		fno->fsize = LD_u32(dir+DIR_FileSize);	/* Size */
		fno->fdate = LD_u16(dir+DIR_WrtDate);		/* Date */
		fno->ftime = LD_u16(dir+DIR_WrtTime);		/* Time */
	}
	*p = 0;
}
#endif /* _USE_DIR */
...
)
{
	...
	if ((u8)*path <= ' ') {			/* Null path means the root directory */
		...
		for (;;) {
			...
			dj->sclust =
				((u32)LD_u16(dir+DIR_FstClusHI) << 16) | 
				LD_u16(dir+DIR_FstClusLO);
		}
	}
...
)
{
	if (disk_readp(buf, sect, 510, 2))		/* Read the boot sector */
		return 3;
	if (LD_u16(buf) != 0xAA55)				/* Check record signature */
		return 2;

	if (!disk_readp(buf, sect, BS_FilSysType, 2) && LD_u16(buf) == 0x4146)	/* Check FAT12/16 */
		return 0;
	if (!disk_readp(buf, sect, BS_FilSysType32, 2) && LD_u16(buf) == 0x4146)	/* Check FAT32 */
		return 0;
	return 1;
}
...
)
{
	...
	if (fmt == 1) {						/* Not an FAT boot record, it may be FDISK format */
		...
		if (disk_readp(buf, bsect, MBR_Table, 16)) {	/* 1st partition entry */
			...
			if (buf[4]) {					/* Is the partition existing? */
				bsect = LD_u32(&buf[8]);	/* Partition offset in LBA */
	...
	fsize = LD_u16(buf+BPB_FATSz16-13);				/* Number of sectors per FAT */
	if (!fsize) fsize = LD_u32(buf+BPB_FATSz32-13);

	fsize *= buf[BPB_NumFATs-13];						/* Number of sectors in FAT area */
	fs->fatbase = bsect + LD_u16(buf+BPB_RsvdSecCnt-13); /* FAT start sector (lba) */
	fs->csize = buf[BPB_SecPerClus-13];					/* Number of sectors per cluster */
	fs->n_rootdir = LD_u16(buf+BPB_RootEntCnt-13);		/* Nmuber of root directory entries */
	tsect = LD_u16(buf+BPB_TotSec16-13);				/* Number of sectors on the file system */
	if (!tsect) tsect = LD_u32(buf+BPB_TotSec32-13);
	mclst = (tsect						/* Last cluster# + 1 */
		- LD_u16(buf+BPB_RsvdSecCnt-13) - fsize - fs->n_rootdir / 16
		) / fs->csize + 2;
	...
	if (fmt == FS_FAT32)
		fs->dirbase = LD_u32(buf+(BPB_RootClus-13));	/* Root directory start cluster */
	else
...
)
{
	...
	fs->org_clust =						/* File start cluster */
		((u32)LD_u16(dir+DIR_FstClusHI) << 16) |
		LD_u16(dir+DIR_FstClusLO);
	fs->fsize = LD_u32(dir+DIR_FileSize);	/* File size */
