...
process_getscantable_idx(int table_idx)
{
    ...
    t_u8 scan_rsp_buf[500];     /* Stack buffer can be as large as ioctl allows 
                                 */
    t_u16 cap_info;
    ...
    iwr.u.data.length = sizeof(scan_rsp_buf);
...
process_setuserscan(int argc, char *argv[])
{
    ...
    for (arg_idx = 0; arg_idx < argc; arg_idx++) {
        if (strncmp(argv[arg_idx], "chan=", strlen("chan=")) == 0) {
             ...
             */
            scan_req.bss_type = atoi(argv[arg_idx] + strlen("type="));
            switch (scan_req.bss_type) {
            ...
            case WLAN_SCAN_BSS_TYPE_ANY:
                ...
                scan_req.bss_type = WLAN_SCAN_BSS_TYPE_ANY;
            }
        }
    }
