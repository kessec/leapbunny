...
#endif
        if (handle->proc_mwlan) {
            r = create_proc_entry("config", 0644, handle->proc_mwlan);
            if (r) {
                r->data = handle;
...
#endif
            remove_proc_entry("config", handle->proc_mwlan);
