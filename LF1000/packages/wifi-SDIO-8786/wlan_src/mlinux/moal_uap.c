...
woal_uap_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    int ret = 0;
    ENTER();
    PRINTM(MCMND, "uap_do_ioctl: ioctl cmd = 0x%x\n", cmd);
    switch (cmd) {
    case UAP_HOSTCMD:
...
/** 
 *  @brief Send get MAC address request to MLAN
 *   
 ...
 */
mlan_status
woal_request_get_mac_address(moal_private * priv)
{
...
woal_uap_get_version(moal_private * priv, char *version, int max_len)
{
    ...
    if (status == MLAN_STATUS_SUCCESS) {
        PRINTM(MCMND, "MOAL UAP VERSION: %s\n",
