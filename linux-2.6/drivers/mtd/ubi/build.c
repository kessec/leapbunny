...
static struct device_attribute dev_mtd_name =
	__ATTR(mtd_name, S_IRUGO, dev_attribute_show, NULL);
...
static int ubi_sysfs_init(struct ubi_device *ubi)
{
	...
	if (err)
		return err;
	err = device_create_file(&ubi->dev, &dev_mtd_name);
	if (err)
		return err;
...
static void ubi_sysfs_close(struct ubi_device *ubi)
{
	...
	device_remove_file(&ubi->dev, &dev_mtd_name);
...
#ifdef CONFIG_MTD_NAND_LF1000_STRESS_TEST
	printk( "%s: Dirty blocks:\n", ubi->mtd->name );
	print_pebs(ubi, si, &si->erase);
#endif
