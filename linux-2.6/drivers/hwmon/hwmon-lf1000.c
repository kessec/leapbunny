...
#define	EXT_POWER_PORT	GPIO_PORT_C	/* external power detect */
#define EXT_POWER_PIN	GPIO_PIN14
...
struct lf1000_hwmon {
	...
	unsigned request_battery_report;	/* report if battery status changes */

	struct platform_device *pdev;

	struct work_struct battery_work;	/* monitor power */
	struct workqueue_struct *power_tasks;
...
int lf1000_get_battery_mv(struct lf1000_hwmon *hwmon_dev)
{
	int reading;
		...
		struct lf1000_hwmon *hwmon_dev, int mv)
{
	...
	if(hwmon_dev->have_external) /* no need to make measurements */
		return EXTERNAL;
...
/*
 * Rearm power button if released, shutdown if pressed for SHUTDOWN_SECS
 */

static void power_button_monitor_task(unsigned long data)
{
	struct lf1000_hwmon *priv = (struct lf1000_hwmon *)data;

	if (hwmon_dev->power_button_count < POWER_BUTTON_COUNT &&
	    !gpio_get_val(POWER_PORT, POWER_PIN)) { /* power button released  */
		gpio_set_int(POWER_PORT, POWER_PIN, 1); /* reenable interrupt */
		input_report_key(hwmon_dev->input, KEY_POWER, 0); /* released */
		dev_info(&priv->pdev->dev, "Power button released\n");
	...
	if (hwmon_dev->power_button_count++ <= POWER_BUTTON_COUNT) {
		...
		hwmon_dev->power_button_timer.function =
						power_button_monitor_task;
		hwmon_dev->power_button_timer.data = data;
					...
					void *priv) 
{
	if(gpio_get_pend(POWER_PORT, POWER_PIN)) {
		...
		if (!hwmon_dev->power_button_count) {/* run background task */
			hwmon_dev->power_button_timer.expires =
				 get_jiffies_64() + POWER_BUTTON_SAMPLING_J;
			hwmon_dev->power_button_timer.function =
						power_button_monitor_task;
			hwmon_dev->power_button_timer.data =
				(unsigned long)priv;
			add_timer(&hwmon_dev->power_button_timer);
		}
...
static void lf1000_set_battery(struct work_struct *work)
{
	...
	if(gpio_have_gpio_k2())	     /* K2 hack: always external power */
		hwmon_dev->have_external = 1;
	else
		hwmon_dev->have_external =
			gpio_get_val(EXT_POWER_PORT, EXT_POWER_PIN);
#endif
	...
	case LOW_BATTERY:
	    ...
	    if ((last_status != LOW_BATTERY && last_status != CRITICAL_BATTERY) ||
		(last_status != LOW_BATTERY && hwmon_dev->request_battery_report)) {
...
static void battery_monitoring_task(unsigned long data)
{
	...
	queue_work(priv->power_tasks, &priv->battery_work);
...
static int lf1000_power_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_MACH_LF_LF1000
	gpio_set_fn(EXT_POWER_PORT, EXT_POWER_PIN, GPIO_GPIOFN);
	if(gpio_have_gpio_k2())	     /* K2 hack: always external power */
		hwmon_dev->have_external = 1;
	else
		hwmon_dev->have_external =
	...
	hwmon_dev->have_external = 1; /* always on external */
#endif
	...
	dev_info(&pdev->dev, "%s.%d have_external is: %d\n",
			__FUNCTION__, __LINE__, priv->have_external);
	...
	priv->power_tasks = create_singlethread_workqueue("power tasks");
	...
	setup_timer(&priv->power_button_timer, power_button_monitor_task,
		       (unsigned long)priv);
...
static int lf1000_power_remove(struct platform_device *pdev)
{
	...
	destroy_workqueue(priv->power_tasks);
