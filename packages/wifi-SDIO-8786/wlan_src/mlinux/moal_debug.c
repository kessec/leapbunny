...
/** Private debug data */
static struct debug_data_priv uap_items_priv;
#endif /* UAP_SUPPORT */
...
woal_debug_entry(moal_private * priv)
{
    ...
    if (priv->bss_type == MLAN_BSS_TYPE_UAP) {
        uap_items_priv.priv = priv;
        uap_items_priv.items = uap_items;
        uap_items_priv.num_of_items = sizeof(uap_items) / sizeof(uap_items[0]);
        uap_items[uap_items_priv.num_of_items - 1].addr +=
            (t_u32) (priv->phandle);
        uap_items[uap_items_priv.num_of_items - 2].addr +=
            (t_u32) (priv->phandle);
        uap_items[uap_items_priv.num_of_items - 3].addr +=
            (t_u32) (priv->phandle);
        uap_items[uap_items_priv.num_of_items - 4].addr +=
            (t_u32) (priv->phandle);
        uap_items[uap_items_priv.num_of_items - 5].addr +=
            (t_u32) (priv->phandle);
    }
#endif
    /* Create proc entry */
