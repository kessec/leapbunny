...
const char driver_version[] =
    "SD8786-%s-M2612" MLAN_RELEASE_VERSION "-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
    " ";
...
#define NETIF_F_DYNALLOC 16
    ...
    priv->media_connected = MFALSE;
    memset(&priv->current_addr, 0, sizeof(priv->current_addr));
    status = woal_request_get_mac_address(priv);
...
woal_init_priv(moal_private * priv)
{
    ...
    if (priv->bss_type == MLAN_BSS_TYPE_UAP) {
        priv->media_connected = MFALSE;
        memset(&priv->current_addr, 0, sizeof(priv->current_addr));
        woal_request_get_mac_address(priv);
    }
#endif
    LEAVE();
    return;
...
woal_cleanup_module(void)
{
    ...
    if (handle->is_suspended == MTRUE) {
        woal_sdio_resume(&(((struct sdio_mmc_card *) handle->card)->func)->dev);
    }
#endif /* SDIO_SUSPEND_RESUME */
