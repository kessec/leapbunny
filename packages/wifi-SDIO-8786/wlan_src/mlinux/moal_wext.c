...
woal_set_powermode(moal_private * priv, char *powermode)
{
    ...
    if (*powermode == '0') {
        PRINTM(MCMND, "Disable power save\n");
        pm_cfg->param.ps_mode = 0;
    } else if (*powermode == '1') {
        PRINTM(MCMND, "Enable power save\n");
        pm_cfg->param.ps_mode = 1;
    } else {
              ...
              struct iw_point *dwrq, char *extra)
{
    ...
    PRINTM(MCMND, "SIOCSIWPRIV requst = %s\n", buf);
    ...
    PRINTM(MCMND, "PRIV Command return: %s, length=%d\n", buf, len);
    dwrq->length = len;
