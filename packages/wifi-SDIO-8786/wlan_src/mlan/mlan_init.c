...
wlan_init_priv(pmlan_private priv)
{
    ...
    priv->wmm_qosinfo = 0;
    priv->gen_null_pkt = MFALSE;        /* Disable NULL Pkg generation by
                                           default */
    ...
    if (priv->bss_type == MLAN_BSS_TYPE_UAP)
        util_init_list_head(&priv->sta_list, MTRUE,
                            pmadapter->callbacks.moal_init_lock);
#endif /* UAP_SUPPORT */
...
wlan_init_adapter(pmlan_adapter pmadapter)
{
    ...
    if (pmadapter->psleep_cfm) {
        ...
        memset(&sleep_cfm_buf->ps_cfm_sleep, 0, sizeof(OPT_Confirm_Sleep));
