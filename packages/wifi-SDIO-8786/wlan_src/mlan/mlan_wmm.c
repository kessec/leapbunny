 ...
 * and will be used to copy back the default values to tos_to_tid in case of 
 * disconnect.
 */
...
wlan_wmm_init(pmlan_adapter pmadapter)
{
    ...
    for (j = 0; j < MLAN_MAX_BSS_NUM; ++j) {
        if ((priv = pmadapter->priv[j])) {
            ...
            util_scalar_init(&priv->wmm.tx_pkts_queued, 0,
                             priv->wmm.ra_list_spinlock,
                             pmadapter->callbacks.moal_init_lock);
            util_scalar_init(&priv->wmm.highest_queued_prio, HIGH_PRIO_TID,
                             priv->wmm.ra_list_spinlock,
                             pmadapter->callbacks.moal_init_lock);
        }
    }
...
wlan_wmm_lists_empty(pmlan_adapter pmadapter)
{
    ...
    for (j = 0; j < MLAN_MAX_BSS_NUM; ++j) {
        if ((priv = pmadapter->priv[j])) {
            ...
            if (util_scalar_read(&priv->wmm.tx_pkts_queued,
...
wlan_wmm_cleanup_queues(pmlan_private priv)
{
    ...
    util_scalar_write(&priv->wmm.tx_pkts_queued, 0, MNULL, MNULL);
    util_scalar_write(&priv->wmm.highest_queued_prio,
...
wlan_wmm_cleanup_node(pmlan_private priv)
{
    ...
    util_free_list_head(&priv->rx_reorder_tbl_ptr,
                        priv->adapter->callbacks.moal_free_lock);
    util_scalar_free(&priv->wmm.tx_pkts_queued,
                     priv->adapter->callbacks.moal_free_lock);
    util_scalar_free(&priv->wmm.highest_queued_prio,
                     priv->adapter->callbacks.moal_free_lock);
}
...
wlan_wmm_add_buf_txqueue(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
    ...
    if (!ra_list) {
        PRINTM(MWARN, "Drop packet, ra_list=%p, media_connected=%d\n", ra_list,
...
//      PRINTM(MERROR, "[+%d->%u] ", pmbuf->data_len, ra_list->total_pkts_size);
    ...
    util_scalar_increment(&priv->wmm.tx_pkts_queued, MNULL, MNULL);
    /* if highest_queued_prio < prio(tid_down), set it to prio(tid_down) */
    util_scalar_conditional_write(&priv->wmm.highest_queued_prio,
...
wlan_ret_wmm_get_status(pmlan_private priv, const HostCmd_DS_COMMAND * resp)
{
    ...
    t_u32 resp_len = resp->size, tlv_len;
    int valid = MTRUE;
                                  ...
                                  pmlan_private * priv, int *tid)
{
    ...
    for (j = MLAN_MAX_BSS_NUM - 1; j >= 0; --j) {
        ...
        do {
            ...
            pmadapter->callbacks.moal_spin_lock(priv_tmp->wmm.ra_list_spinlock);

            for (i =
                 util_scalar_read(&priv_tmp->wmm.highest_queued_prio, MNULL,
                                  MNULL); i >= LOW_PRIO_TID; --i) {

                tid_ptr = &(priv_tmp)->wmm.tid_tbl_ptr[tos_to_tid[i]];
                if (!util_peek_list(&tid_ptr->ra_list, MNULL, MNULL))
                    continue;
                ...
                do {
                    if (util_peek_list(&ptr->buf_head, MNULL, MNULL)) {
                        /* if highest_queued_prio > i, set it to i */
                        util_scalar_conditional_write(&priv_tmp->wmm.
                                                      highest_queued_prio,
                                                      MLAN_SCALAR_COND_GREATER_THAN,
                                                      i, i, MNULL, MNULL);
                        *priv = priv_tmp;
                        *tid = tos_to_tid[i];
                        /* keep holding priv->ra_list_spinlock to maintain ptr */
                        LEAVE();
                        return ptr;
                    }
            ...
            pmadapter->callbacks.moal_spin_unlock(priv_tmp->wmm.
                                                  ra_list_spinlock);
...
wlan_send_single_packet(pmlan_private priv, raListTbl * ptr, int ptrindex)
{
    ...
    if ((pmbuf = (pmlan_buffer) util_dequeue_list(&ptr->buf_head,
        ...
        if (status == MLAN_STATUS_RESOURCE) {
            ...
            util_scalar_decrement(&priv->wmm.tx_pkts_queued, MNULL, MNULL);
            pmadapter->callbacks.moal_spin_unlock(priv->wmm.ra_list_spinlock);
        }
    } else {
        pmadapter->callbacks.moal_spin_unlock(priv->wmm.ra_list_spinlock);
...
wlan_send_processed_packet(pmlan_private priv, raListTbl * ptr, int ptrindex)
{
    ...
    if ((pmbuf = (pmlan_buffer) util_dequeue_list(&ptr->buf_head,
                                                  MNULL, MNULL))) {
        pmbuf_next = (pmlan_buffer) util_peek_list(&ptr->buf_head,
                                                   MNULL, MNULL);
        pmadapter->callbacks.moal_spin_unlock(priv->wmm.ra_list_spinlock);
        tx_param.next_pkt_len =
            ((pmbuf_next) ? pmbuf_next->data_len + sizeof(TxPD) : 0);
