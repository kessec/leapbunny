                      ...
                      IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    /* type, len are already endian converted by application */
    pmgmt_ie_list->ds_mgmt_ie.type = cust_ie->type;
    pmgmt_ie_list->ds_mgmt_ie.len = cust_ie->len;
    payload_len = wlan_le16_to_cpu(cust_ie->len);

    cptr = (custom_ie *) cust_ie->ie_data_list;
    pptr = pmgmt_ie_list->ds_mgmt_ie.ie_data_list;
    if (cptr && pptr)
        memcpy(pptr, cptr, payload_len);

    cmd->size -= MAX_MGMT_IE_INDEX * sizeof(custom_ie);
    cmd->size += payload_len;
    cmd->size = wlan_cpu_to_le16(cmd->size);
