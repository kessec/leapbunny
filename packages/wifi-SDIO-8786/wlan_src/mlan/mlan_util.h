 ...
 */
static INLINE t_void
util_scalar_init(pmlan_scalar pscalar,
                 t_s32 val,
                 t_void * plock_to_use,
                 mlan_status(*moal_init_lock) (t_void ** pplock))
{
 ...
 */
static INLINE t_void
util_scalar_free(pmlan_scalar pscalar,
                 mlan_status(*moal_free_lock) (t_void * plock))
{
    if (pscalar->flags & MLAN_SCALAR_FLAG_UNIQUE_LOCK)
        moal_free_lock(&pscalar->plock);
}
 ...
 */
static INLINE t_s32
util_scalar_read(pmlan_scalar pscalar,
                 mlan_status(*moal_spin_lock) (t_void * plock),
                 mlan_status(*moal_spin_unlock) (t_void * plock))
{
    t_s32 val;
    ...
    if (moal_spin_lock)
        moal_spin_lock(pscalar->plock);
    val = pscalar->value;
    if (moal_spin_unlock)
        moal_spin_unlock(pscalar->plock);
 ...
 */
static INLINE t_void
util_scalar_write(pmlan_scalar pscalar,
                  t_s32 val,
                  mlan_status(*moal_spin_lock) (t_void * plock),
                  mlan_status(*moal_spin_unlock) (t_void * plock))
{
 ...
 */
static INLINE t_void
util_scalar_increment(pmlan_scalar pscalar,
                      mlan_status(*moal_spin_lock) (t_void * plock),
                      mlan_status(*moal_spin_unlock) (t_void * plock))
{
    if (moal_spin_lock)
 ...
 */
static INLINE t_void
util_scalar_decrement(pmlan_scalar pscalar,
                      mlan_status(*moal_spin_lock) (t_void * plock),
                      mlan_status(*moal_spin_unlock) (t_void * plock))
{
    if (moal_spin_lock)
 ...
 */
static INLINE t_s32
util_scalar_offset(pmlan_scalar pscalar,
                   t_s32 offset,
                   mlan_status(*moal_spin_lock) (t_void * plock),
                   mlan_status(*moal_spin_unlock) (t_void * plock))
{
 ...
 */
static INLINE t_u8
util_scalar_conditional_write(pmlan_scalar pscalar,
