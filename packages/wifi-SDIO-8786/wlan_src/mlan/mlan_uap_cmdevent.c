                       ...
                       IN t_u16 cmd_action, IN pmlan_ioctl_req pioctl_buf)
{
    ...
    if (memcmp(zero_mac, &bss->param.bss_config.mac_addr, MLAN_MAC_ADDR_LENGTH)) {
    ...
    if ((bss->param.bss_config.protocol == PROTOCOL_WPA) ||
        ...
        if ((bss->param.bss_config.wep_cfg.key0.length) &&
            ...
            memcpy(tlv_wep_key->key, bss->param.bss_config.wep_cfg.key0.key,
        ...
        if ((bss->param.bss_config.wep_cfg.key1.length) &&
            ...
            memcpy(tlv_wep_key->key, bss->param.bss_config.wep_cfg.key1.key,
        ...
        if ((bss->param.bss_config.wep_cfg.key2.length) &&
            ...
            memcpy(tlv_wep_key->key, bss->param.bss_config.wep_cfg.key2.key,
        ...
        if ((bss->param.bss_config.wep_cfg.key3.length) &&
            ...
            memcpy(tlv_wep_key->key, bss->param.bss_config.wep_cfg.key3.key,
                           ...
                           IN t_u16 cmd_action, IN pmlan_ioctl_req pioctl_buf)
{
    ...
    if (pioctl_buf->req_id == MLAN_IOCTL_BSS) {
        ...
        if (bss->sub_command == MLAN_OID_BSS_MAC_ADDR) {
            ...
            LEAVE();
            return wlan_uap_cmd_ap_config(pmpriv, cmd, cmd_action, pioctl_buf);
        }
        ...
        if ((misc->sub_command == MLAN_OID_MISC_CUSTOM_IE) &&
            ...
            memcpy(ie, misc->param.cust_ie.ie_data_list,
                   misc->param.cust_ie.len);
        }
    }
    LEAVE();
                           ...
                           IN mlan_ioctl_req * pioctl_buf)
{
    ...
    while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
        ...
        case TLV_TYPE_SSID:
            ...
            memcpy(bss->param.bss_config.ssid.ssid, tlv_ssid->ssid, tlv_len);
            break;
        ...
        case TLV_TYPE_RATES:
            tlv_rates = (MrvlIEtypes_RatesParamSet_t *) tlv;
            memcpy(bss->param.bss_config.rates, tlv_rates->rates, tlv_len);
            break;
        ...
        case TLV_TYPE_UAP_STA_MAC_ADDR_FILTER:
            ...
            bss->param.bss_config.filter.filter_mode =
                tlv_mac_filter->filter_mode;
            memcpy((t_u8 *) bss->param.bss_config.filter.mac_list,
        ...
        case TLV_TYPE_UAP_WEP_KEY:
            ...
            if (pkey) {
                ...
                memcpy(pkey->key, tlv_wep_key->key, pkey->length);
            }
            break;
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        memcpy(info->param.ver_ext.version_str, sys_info->sys_info,
               sizeof(char) * 64);
    }
    LEAVE();
    return MLAN_STATUS_SUCCESS;
                        ...
                        IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    memcpy(pcmd_sta_deauth->mac, deauth->mac_addr, MLAN_MAC_ADDR_LENGTH);
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        for (i = 0; i < info->param.sta_list.sta_count; i++) {
            memcpy(info->param.sta_list.info[i].mac_address, tlv->mac_address,
                   MLAN_MAC_ADDR_LENGTH);
...
wlan_get_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    if (!(sta_ptr = (sta_node *) util_peek_list(&priv->sta_list,
    ...
    while (sta_ptr != (sta_node *) & priv->sta_list) {
        if (!memcmp(sta_ptr->mac_addr, mac, MLAN_MAC_ADDR_LENGTH)) {
            LEAVE();
            return sta_ptr;
        }
...
wlan_add_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    if (priv->adapter->callbacks.
        moal_malloc(sizeof(sta_node), (t_u8 **) & sta_ptr)) {
        PRINTM(MERROR, "station node memory allocation failed\n");
        return MNULL;
    }
    memset(sta_ptr, 0, sizeof(sta_node));
    memcpy(sta_ptr->mac_addr, mac, MLAN_MAC_ADDR_LENGTH);
    util_enqueue_list_tail(&priv->sta_list,
...
wlan_delete_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    if ((sta_ptr = wlan_get_station_entry(priv, mac))) {
        util_unlink_list(&priv->sta_list,
                         (pmlan_linked_list) sta_ptr,
                         priv->adapter->callbacks.moal_spin_lock,
                         priv->adapter->callbacks.moal_spin_unlock);
        priv->adapter->callbacks.moal_mfree((t_u8 *) sta_ptr);
    }
    return;
}
...
wlan_delete_station_list(pmlan_private priv)
{
    ...
    while ((sta_ptr = (sta_node *) util_dequeue_list(&priv->sta_list,
                                                     priv->adapter->callbacks.
                                                     moal_spin_lock,
                                                     priv->adapter->callbacks.
                                                     moal_spin_unlock))) {
...
wlan_check_11n_capability(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == MRVL_MGMT_FRAME_TLV_ID) {
...
wlan_check_uap_capability(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == MRVL_PKT_FWD_CTL_TLV_ID) {
...
mlan_process_uap_event(IN t_void * priv)
{
    ...
    memcpy(pmbuf->pbuf + pmbuf->data_offset, (t_u8 *) & eventcause,
           sizeof(eventcause));
    ...
    case EVENT_MICRO_AP_STA_ASSOC:
        memcpy(sta_addr, pmadapter->event_body + 2, MLAN_MAC_ADDR_LENGTH);
    ...
    case EVENT_MICRO_AP_STA_DEAUTH:
        memcpy(sta_addr, pmadapter->event_body + 2, MLAN_MAC_ADDR_LENGTH);
        pmadapter->callbacks.moal_spin_lock(pmpriv->wmm.ra_list_spinlock);
        if (pmpriv->is_11n_enabled) {
            wlan_uap_11n_cleanup_txbastream_tbl(pmpriv, sta_addr);
            wlan_uap_11n_cleanup_reorder_tbl(pmpriv, sta_addr);
        }
        pmadapter->callbacks.moal_spin_unlock(pmpriv->wmm.ra_list_spinlock);
    ...
    if (pevent->event_id) {
        ...
        memcpy((t_u8 *) pevent->event_buf, pmbuf->pbuf + pmbuf->data_offset,
               pevent->event_len);
