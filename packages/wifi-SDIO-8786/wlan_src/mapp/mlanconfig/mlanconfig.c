...
process_custom_ie(int argc, char *argv[])
{
    ...
    if (argc == 3 || argc == 4) {
        if (argc == 3)
            ...
            ie_ptr->ie_index = (t_u16) cpu_to_le16(atoi(argv[3]));
        }
    } else {
        /* Locate headers */
        ie_ptr = (custom_ie *) (tlv->ie_data);
        ...
        ie_ptr->ie_index = cpu_to_le16(atoi(argv[3]));
        ie_ptr->mgmt_subtype_mask = cpu_to_le16(mgmt_subtype_mask);
        ie_ptr->ie_length = cpu_to_le16(ie_len);
        if (argc == 6)
    ...
    if (argc > 4) {
        ...
        while ((unsigned int) ie_len >= sizeof(custom_ie)) {
            printf("Index [%d]\n", le16_to_cpu(ie_ptr->ie_index));
            if (ie_ptr->ie_length)
                printf("Management Subtype Mask = 0x%02x\n",
                       le16_to_cpu(ie_ptr->mgmt_subtype_mask) == 0 ?
                       CUSTOM_IE_AUTO_MASK :
                       le16_to_cpu(ie_ptr->mgmt_subtype_mask));
            else
                printf("Management Subtype Mask = 0x%02x\n",
                       le16_to_cpu(ie_ptr->mgmt_subtype_mask));
            hexdump("IE Buffer", (void *) ie_ptr->ie_buffer,
                    le16_to_cpu(ie_ptr->ie_length), ' ');
            ie_len -= sizeof(custom_ie) + le16_to_cpu(ie_ptr->ie_length);
