...
const char driver_version[] =
    "USB8786-%s-" MLAN_RELEASE_VERSION "-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
    " ";
...
woal_close(struct net_device *dev)
{
    ...
    MODULE_PUT;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
    /* Setting 'pm_usage_count' to 0 in close */
    usb_autopm_enable(((struct usb_card_rec *) priv->phandle->card)->intf);
#endif /* >= 2.6.24 */
 ...
 */
t_void
woal_main_work(struct work_struct * work)
{
...
#define NETIF_F_DYNALLOC 16
    ...
    MLAN_INIT_WORK(&handle->main_work, woal_main_work);
