...
#define LAYER0	"/dev/layer0"
#define LAYER1	"/dev/layer1"
#define MLC	"/dev/mlc"
#define TS_NAME	"LF1000 touchscreen interface"
#define KB_NAME	"LF1000 Keyboard"

#define TSLIB_COLOR	0x7fff0000	// RED
#define INPUT_COLOR	0x7f0000FF	// BLUE

int hstride = -1, vstride = -1;
int base0, base1, fbsize, bufsize;
int ssi, hsize, vsize;
void *fb0 = (void *)-1;
void *fb1 = (void *)-1;
struct screensize_cmd ss;
int layer0, layer1, mlc;
unsigned char *dc;
...
int main (int argc, char **argv)
{
	...
	// Set up frame buffer
	layer0 = open(LAYER0, O_RDWR|O_SYNC);
	if(layer0 < 0)
	{
		perror("failed to open layer0 device");
		return 1;
	}
	layer1 = open(LAYER1, O_RDWR|O_SYNC);
	if(layer1 < 0)
	{
		perror("failed to open layer1 device");
		return 1;
	}
	mlc = open(MLC, O_RDWR);
	if(mlc < 0)
	...
	base0 = ioctl(layer0, MLC_IOCQADDRESS, 0);
	if(base0 == -EFAULT) {
		perror("get_address ioctl failed");
		return 1;
	}
	base1 = ioctl(layer1, MLC_IOCQADDRESS, 0);
	if(base1 == -EFAULT) {
		perror("get_address ioctl failed");
		return 1;
	}
	/* figure out the size of the frame buffer */
	fbsize = ioctl(layer0, MLC_IOCQFBSIZE, 0);
	if(fbsize < 0) {
		perror("get_fbsize ioctl failed");
		return 1;
	}
	hstride = ioctl(layer0, MLC_IOCQHSTRIDE, 0);
	if(hstride < 0) {
		perror("get_hstride ioctl failed");
		return 1;
	}

	vstride = ioctl(layer0, MLC_IOCQVSTRIDE, 0);
	if(vstride < 0) {
	...
	if (1 || vstride != hsize * hstride)
	{
		...
		int x = ioctl(layer0, MLC_IOCTVSTRIDE, vstride);
		if(x < 0) {
		...
		pos.right = hsize;
		pos.bottom = vsize;
		x = ioctl(layer0, MLC_IOCSPOSITION, &pos);
		if(x < 0) {
			perror("set positio ioctl failed");
			return 1;
		}
		x = ioctl(layer0, MLC_IOCTDIRTY, 1);
		if(x < 0) {
	...
	fb0 = mmap(0, fbsize, PROT_READ | PROT_WRITE, MAP_SHARED, layer0, 
				   (unsigned int)base0);
	fb1 = mmap(0, fbsize, PROT_READ | PROT_WRITE, MAP_SHARED, layer1, 
				   (unsigned int)base1);
	if(fb0 == MAP_FAILED) {
	...
	if(fb1 == MAP_FAILED) {
		perror("mmap() fm1 failed");
		return 1;
	}

	printf ("fb=%08lx,%08lx hstride=%d vstride=%d fbsize=%d ss.w=%d ss.h=%d\n", 
		(unsigned long)fb0, (unsigned long)fb1,
		hstride, vstride, fbsize,
		ss.width, ss.height);

	// Clear screen
	memset ((unsigned char *)fb0, 0xff, hstride*hsize*vsize);
	memset ((unsigned char *)fb1, 0xff, hstride*hsize*vsize);
