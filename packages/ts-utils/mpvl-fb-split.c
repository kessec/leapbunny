...
int main (int argc, char **argv)
{
	...
	while (1) {
		...
		for (i=0; i<ret; i++)
		{
			...
			{
				int x = (samp[i].x % split) + split;
				int y = samp[i].y;
				fatdot (x, y, TSLIB_COLOR);
				if (tlx != -1 && tly != -1)
					line (tlx, tly, x, y, TSLIB_COLOR);
				tlx = x;
				tly = y;
			}
			if (!samp[i].pressure)
		...
		if (poll (polldat, 2, 0) > 0)
		{
			...
			for (i = 0; i < rd / sizeof(struct input_event); i++)
			{
				...
				case EV_SYN: 
					...
					{
						int myx = (x % split);
						fatdot(myx, y, INPUT_COLOR);
						if (ilx != -1 && ily != -1)
							line (ilx, ily, myx, y, INPUT_COLOR);
						ilx = myx;
						ily = y;
					}
					break;
