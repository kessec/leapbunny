/* vnotify.c -- Post an event on the video_events_socket to tell the
 *              application layer about a video event.
 */
...
#define VIDEO_PAYLOAD	4
#define VIDEO_SOCK		"/tmp/video_events_socket"
...
int main(void)
{
	...
	sock = create_report_socket(VIDEO_SOCK);
	if (sock < 0) {
