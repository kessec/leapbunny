...
void dpc_init(u8 thb, u8 thf, u8 tvb, u8 tvf)
{
	...
	DPC16(DPCHTOTAL)  = DPC_HSYNC_SWIDTH + thf + thb + X_RESOLUTION - 1;
	DPC16(DPCHSWIDTH) = DPC_HSYNC_SWIDTH - 1;
	DPC16(DPCHASTART) = DPC_HSYNC_SWIDTH + thb - 1;
	DPC16(DPCHAEND)   = DPC_HSYNC_SWIDTH + thb + X_RESOLUTION - 1;

	/*
	 * Set up vertical sync
	 */

	DPC16(DPCVTOTAL)  = DPC_VSYNC_SWIDTH + tvf + tvb + Y_RESOLUTION - 1;
	DPC16(DPCVSWIDTH) = DPC_VSYNC_SWIDTH - 1;
	DPC16(DPCVASTART) = DPC_VSYNC_SWIDTH + tvb - 1;
	DPC16(DPCVAEND)   = DPC_VSYNC_SWIDTH + tvb + Y_RESOLUTION - 1;
